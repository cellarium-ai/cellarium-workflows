...
updated_config = setup_gcs_output_handling(config)
...
try:
    if capture_logs:
        ...
        with capture_logs_to_files(stdout_file, stderr_file):
            cellarium_ml_cli(args=[tool, subcommand, "--config", updated_config])
            
        print(f"✅ Logs captured and will be synced to GCS")
    else:
